function tabBGTracker(el_) {
  el_.appendChild(secTitle('??','BG Patient Tracker','Continuous blood glucose monitoring — Premium Feature','#a855f7'));
  var token = null;
  try {
    ...
    for (var i=0;i<keys.length;i++) {
      if (keys[i].indexOf('supabase')>-1||keys[i].indexOf('auth-token')>-1) {
        ...
        if (v && v.access_token) { token = v.access_token; break; }
      }
    }
  } catch(e) {}
  var d = document.createElement('div');
  d.style.cssText = 'margin:20px 0;border-radius:20px;padding:36px 24px;text-align:center;background:rgba(168,85,247,0.06);border:1px solid rgba(168,85,247,0.2)';
  if (token) {
    d.innerHTML = '<div style="font-size:44px;margin-bottom:14px">??</div><h2 style="color:#f1f5f9;font-size:20px;font-weight:700;margin:0 0 10px">BG Patient Tracker</h2><p style="color:#94a3b8;font-size:14px;margin:0 0 22px">You are signed in. Open your dashboard to manage patients and view BG trends with AI analysis.</p><a href="/dashboard" style="display:inline-block;background:#a855f7;color:#fff;padding:13px 30px;border-radius:12px;text-decoration:none;font-weight:700;font-size:14px">Open Dashboard \u2192</a>';
  } else {
    d.innerHTML = '<div style="font-size:44px;margin-bottom:14px">??</div><h2 style="color:#f1f5f9;font-size:20px;font-weight:700;margin:0 0 10px">BG Patient Tracker</h2><p style="color:#94a3b8;font-size:14px;margin:0 0 16px;line-height:1.6">Monitor your diabetic patients continuously. ADA 2024 classification, estimated HbA1c, hypo/hyperglycaemia alerts, medication response tracking.</p><div style="background:rgba(168,85,247,0.08);border-radius:10px;padding:12px;margin-bottom:22px"><p style="color:#d8b4fe;font-size:13px;margin:0;font-weight:600">From \u20a615,000/month \u00b7 Up to 20 patients \u00b7 Earn \u20a685,000/month net</p></div><a href="/pricing" style="display:inline-block;background:#a855f7;color:#fff;padding:13px 28px;border-radius:12px;text-decoration:none;font-weight:700;font-size:14px;margin-right:10px">Get Premium \u2192</a><a href="/auth" style="display:inline-block;color:#94a3b8;padding:13px 18px;border-radius:12px;text-decoration:none;font-size:13px;border:1px solid rgba(148,163,184,0.2)">Sign In</a>';
  }
  el_.appendChild(d);
}
...
function tabInsulinChart(el_) {
  ...
  el_.appendChild(card('Insulin Classification & Pharmacokinetics','#ef4444','<div class="tbl-wrap"><table><thead><tr style="background:#fef2f2"><th>Type</th><th>Examples</th><th>Onset</th><th>Peak</th><th>Duration</th><th>Use</th></tr></thead><tbody>'+[
  ...
  el_.appendChild(card('Local Insulin Brands Available in Nigeria','#ef4444','<div class="tbl-wrap"><table><thead><tr style="background:#fef2f2"><th>Generic Name</th><th>International Brand</th><th>Common Nigeria Brand/Source</th><th>Type</th></tr></thead><tbody>'+[
  ...
  el_.appendChild(card('Sick Day Rules (Type 1 & Insulin-Treated Type 2)','#ef4444','<div class="tbl-wrap"><table><thead><tr style="background:#fef2f2"><th>Rule</th><th>Detail</th></tr></thead><tbody>'+[
...
function tabDMDetect(el_) {
  ...
  document.getElementById('dm-classify').onclick=function(){
    ...
    var fpgRaw=parseFloat(document.getElementById('dm-fg').value),hba=parseFloat(document.getElementById('dm-h').value),ogttRaw=parseFloat(document.getElementById('dm-og').value),rbgRaw=parseFloat(document.getElementById('dm-rb').value),sx=document.getElementById('dm-sx').checked;
    var fpg=isNaN(fpgRaw)?NaN:toMgDl(fpgRaw), ogtt=isNaN(ogttRaw)?NaN:toMgDl(ogttRaw), rbg=isNaN(rbgRaw)?NaN:toMgDl(rbgRaw);
    ...
    if(!isNaN(rbg)) results.push({test:'RBG',val:rbgRaw+' '+unitLabel,cat:(rbg>=200&&sx)?'Diabetes':'Inconclusive'});
    ...
    var worst=results.reduce(function(a,b){return b.cat==='Diabetes'?'Diabetes':a==='Diabetes'?'Diabetes':b.cat==='Prediabetes'?'Prediabetes':b.cat==='Inconclusive'?a:a;},'Normal');
    var cc={Normal:'#16a34a',Prediabetes:'#d97706',Diabetes:'#dc2626',Inconclusive:'#64748b'},cb={Normal:'#f0fdf4',Prediabetes:'#fffbeb',Diabetes:'#fef2f2',Inconclusive:'#f8fafc'};
    ...
    if(_dmInv.length){
      ...
      _dmInvHtml+='</div>';
      _dmInvHtml+='<div class="tbl-wrap"><table style="width:100%;border-collapse:collapse">';
...
function tabDMEval(el_) {
  ...
  el_.appendChild(card('At Every Visit','#f59e0b','<div class="tbl-wrap"><table><thead><tr style="background:#fffbeb"><th>Test</th><th>Target / Normal</th><th>Action if Abnormal</th></tr></thead><tbody>'+[
  ...
  ].map(function(r){return '<tr class="tbl-row"><td style="font-weight:600">'+r[0]+'</td><td style="color:#d97706;font-weight:600">'+r[1]+'</td><td style="font-size:12px">'+r[2]+'</td></tr>';}).join('')+'</tbody></table></div>'));

  el_.appendChild(card('Every 3 Months (Until Stable) — Then 6-Monthly','#f59e0b','<div class="tbl-wrap"><table><thead><tr style="background:#fffbeb"><th>Test</th><th>Target</th><th>Notes</th></tr></thead><tbody>'+[
  ...
  el_.appendChild(card('Annual Investigations — Full Panel','#f59e0b','<div class="tbl-wrap"><table><thead><tr style="background:#fffbeb"><th>Test</th><th>Target / Normal</th><th>Action if Abnormal</th></tr></thead><tbody>'+[
  ...
  ].map(function(r){return '<tr class="tbl-row"><td style="font-weight:600">'+r[0]+'</td><td style="color:#d97706;font-weight:600;font-size:12px">'+r[1]+'</td><td style="font-size:12px">'+r[2]+'</td></tr>';}).join('')+'</tbody></table></div>'));

  el_.appendChild(card('Additional Tests — T1DM Specific','#dc2626','<div class="tbl-wrap"><table><thead><tr style="background:#fef2f2"><th>Test</th><th>Frequency</th><th>Rationale</th></tr></thead><tbody>'+[
  ...
  ].map(function(r){return '<tr class="tbl-row"><td style="font-weight:600">'+r[0]+'</td><td style="color:#dc2626;font-weight:600">'+r[1]+'</td><td style="font-size:12px">'+r[2]+'</td></tr>';}).join('')+'</tbody></table></div>'));
  ...
  el_.appendChild(card('Drug Management — Day Before & Day of Surgery','#7c3aed','<div class="tbl-wrap"><table><thead><tr style="background:#faf5ff"><th>Drug / Class</th><th>Day Before Surgery</th><th>Morning of Surgery (Fasting)</th><th>Resume After</th></tr></thead><tbody>'+[
  ...
  ].map(function(r){return '<tr class="tbl-row"><td style="font-weight:700;font-size:12px">'+r[0]+'</td><td style="font-size:12px">'+r[1]+'</td><td style="font-size:12px">'+r[2]+'</td><td style="font-size:12px;color:#7c3aed">'+r[3]+'</td></tr>';}).join('')+'</tbody></table></div>'));
  ...
  el_.appendChild(card('VRIII — Variable Rate IV Insulin Infusion (Sliding Scale)','#7c3aed','<div style="font-size:13px;color:#374151">'+
    ...
    '<div style="background:#faf5ff;padding:12px;border-radius:8px;margin-bottom:10px">'+
      ...
      '<div class="tbl-wrap"><table style="width:100%;font-size:12px"><thead><tr style="background:#e9d5ff"><th>Blood Glucose (mmol/L)</th><th>Blood Glucose (mg/dL)</th><th>Infusion Rate</th></tr></thead><tbody>'+[
      ...
      ].map(function(r){var hl=r[2].includes('target');return '<tr class="tbl-row"'+(hl?' style="background:#f0fdf4"':'')+"><td style=\"font-weight:"+(hl?700:400)+";"+(hl?'color:#16a34a':'')+"\">"+r[0]+'</td><td>'+r[1]+'</td><td style="font-size:11px;'+(hl?'color:#16a34a;font-weight:700':'')+'">'+r[2]+'</td></tr>';}).join('')+'</tbody></table></div>'+
    '</div>'+
...
function tabDMT2(el_) {
  ...
  el_.appendChild(card('GLP-1 RA — Agents, Doses, Trials & Key Effects','#16a34a','<div class="tbl-wrap"><table><thead><tr style="background:#f0fdf4"><th>Drug (Brand)</th><th>Route / Freq</th><th>Starting → Max Dose</th><th>Key CV Trial</th><th>HbA1c ↓</th><th>Weight ↓</th><th>Key Notes</th></tr></thead><tbody>'+[
