...
var MOD_META = {
  HTN:    { icon:ICONS.htn,  name:'Hypertension',   sub:'ESC/ESH 2023 · BP classifier · Treatment plans',      color:'#0ea5e9', bg:'rgba(14,165,233,0.12)' },
  HYPO:   { icon:ICONS.hypo,  name:'Hypotension',    sub:'Shock types · MAP · SSC protocols',                   color:'#a855f7', bg:'rgba(168,85,247,0.12)' },
  DM:     { icon:ICONS.bgTracker,  name:'Diabetes',       sub:'ADA 2024 · DKA/HHS · BG Tracker',                    color:'#ef4444', bg:'rgba(239,68,68,0.12)' },
  SEPSIS: { icon:ICONS.sepsis,  name:'Sepsis',         sub:'qSOFA · SOFA · Hour-1 Bundle',                        color:'#f97316', bg:'rgba(249,115,22,0.12)' },
  PAED:   { icon:ICONS.paed,  name:'Paediatrics',    sub:'Weight-based dosing · DKA · Neonatal resus',          color:'#ec4899', bg:'rgba(236,72,153,0.12)' },
  RENAL:  { icon:ICONS.renal,  name:'Renal',          sub:'eGFR · CKD staging · Drug adjustments',               color:'#06b6d4', bg:'rgba(6,182,212,0.12)' },
  DRUG:   { icon:ICONS.drug,  name:'Drug Reference', sub:'Antibiotics · Vasopressors · Electrolytes',           color:'#22c55e', bg:'rgba(34,197,94,0.12)' },
  COMB:   { icon:ICONS.combined_htn,  name:'Combined Overview', sub:'HTN + Hypotension + DM side-by-side',              color:'#64748b', bg:'rgba(100,116,139,0.12)' },
  REVIEW: { icon:ICONS.review,  name:'Reviews',        sub:'Feedback · Glossary',                                 color:'#fbbf24', bg:'rgba(251,191,36,0.1)' },
};
...
document.addEventListener('keydown', function(e) {
  ...
  if (key === 'Escape') {
    ...
    if (_navLevel === 'content') { navBackFromContent(); return; }
    if (_navLevel === 'drugtabs') { navToDrugGroups(); return; }
    if (_navLevel === 'druggroups') { navToHome(); return; }
...
window.addEventListener('popstate', function(e) {
  ...
  else if (st.triad === 'druggroups'){ showDrugGroupsLanding(true); }
  else if (st.triad === 'drugtabs')  { showDrugGroupTabs(st.groupIdx, true); }
...
function showHomeDashboard(fromHistory) {
  ...
  var dgv  = document.getElementById('drug-groups-view');
  var dgvBadge = document.getElementById('dgv-mod-badge');
  if (dgvBadge && MOD_META['DRUG']) {
    dgvBadge.innerHTML = MOD_META['DRUG'].icon;
    dgvBadge.style.color = MOD_META['DRUG'].color;
  }
  var dgtv = document.getElementById('drug-group-tabs-view');
  ...
  if (dgv)  dgv.style.display  = 'none';
  if (dgtv) dgtv.style.display = 'none';
...
function showTabGrid(mod, fromHistory) {
  // DRUG module uses its own two-level landing page
  if (mod === 'DRUG') {
    showDrugGroupsLanding(fromHistory);
    return;
  }
  _navLevel = 'tabgrid';
  _navMod   = mod;
...
function navBackFromContent() {
  // If we came from a drug group tabs view, go back there
  if (_navMod === 'DRUG' && typeof S.drugGroup !== 'undefined') {
    showDrugGroupTabs(S.drugGroup);
  } else if (_navMod) {
    showTabGrid(_navMod);
  } else {
    showHomeDashboard();
...
function navToDrugGroups() {
  showDrugGroupsLanding();
}

function showDrugGroupsLanding(fromHistory) {
  _navLevel = 'druggroups';
  _navMod   = 'DRUG';
  if (!fromHistory) _pushNav({ triad: 'druggroups' });
  ...
  if (hd)   hd.style.display   = 'none';
  if (tgv)  tgv.style.display  = 'none';
  if (dgv)  dgv.style.display  = 'block';
  if (dgtv) dgtv.style.display = 'none';
  if (mc)   mc.style.display   = 'none';
  if (cbb)  cbb.classList.remove('visible');
  if (tn)   tn.style.display   = 'none';
  ...
  // Build group cards
  var container = document.getElementById('drug-group-cards');
  if (!container) return;
  container.innerHTML = '';

  DRUG_SUPER_GROUPS.forEach(function(sg, idx) {
    var card = document.createElement('button');
    card.style.cssText = 'background:#fff;border:2px solid #e2e8f0;border-radius:16px;padding:16px 14px;text-align:left;cursor:pointer;transition:border-color 0.15s,box-shadow 0.15s;display:flex;flex-direction:column;gap:8px;width:100%';
    var tabCount = sg.tabs.length;
    card.innerHTML =
      '<div style="width:40px;height:40px;border-radius:12px;background:' + sg.color + '22;display:flex;align-items:center;justify-content:center;color:' + sg.color + '">' + sg.icon + '</div>' +
      '<div style="font-size:13px;font-weight:800;color:#0f172a;line-height:1.2">' + sg.label + '</div>' +
      '<div style="font-size:11px;color:#64748b;line-height:1.4">' + sg.desc + '</div>' +
      '<div style="margin-top:4px;display:inline-flex;align-items:center;gap:4px;background:' + sg.color + '18;color:' + sg.color + ';padding:3px 10px;border-radius:20px;font-size:10.5px;font-weight:700">' + tabCount + ' tabs &nbsp;→</div>';
    card.onmouseover = function() { this.style.borderColor = sg.color; this.style.boxShadow = '0 4px 16px ' + sg.color + '33'; };
    card.onmouseout  = function() { this.style.borderColor = '#e2e8f0'; this.style.boxShadow = 'none'; };
    card.onclick = (function(i) { return function() { showDrugGroupTabs(i); }; })(idx);
    container.appendChild(card);
  });
...
function showDrugGroupTabs(groupIdx, fromHistory) {
  _navLevel = 'drugtabs';
  _navMod   = 'DRUG';
  S.drugGroup = groupIdx;
  if (!fromHistory) _pushNav({ triad: 'drugtabs', groupIdx: groupIdx });
  ...
  if (tgv)  tgv.style.display  = 'none';
  if (dgv)  dgv.style.display  = 'none';
  if (dgtv) dgtv.style.display = 'block';
  if (mc)   mc.style.display   = 'none';
  if (cbb)  cbb.classList.remove('visible');
  if (tn)   tn.style.display   = 'none';

  var sg = DRUG_SUPER_GROUPS[groupIdx];
  if (!sg) return;

  var badge = document.getElementById('dgtv-badge');
  var label = document.getElementById('dgtv-label');
  var sub   = document.getElementById('dgtv-sub');
  if (badge) { badge.innerHTML = sg.icon; badge.style.background = sg.color + '22'; }
  if (label) label.textContent = sg.label;
  if (sub)   sub.textContent = sg.tabs.length + ' tabs available';

  var container = document.getElementById('drug-group-tab-cards');
  if (!container) return;
  container.innerHTML = '';

  var m = MOD_META['DRUG'] || { color: '#22c55e' };
  sg.tabs.forEach(function(tabName) {
    var idx = TABS.DRUG.indexOf(tabName);
    if (idx === -1) return;
    appendTabCard(container, tabName, idx, 'DRUG', sg.color, sg.label);
  });
...
// ── MODULE → EMERGENCY PROTOCOL MAPPING ────────────────────────
var _modToEmIdx = {
  HTN:    0,
  SEPSIS: 1,
  DM:     null,
...
function showEmProtocol(idx) {
  ...
  var sub = document.querySelector('#emergency-overlay .em-subtitle');
  if (sub) sub.innerHTML = td.icon + ' ' + td.label;
...
window._searchIndex = _searchIndex;
...
function buildSearchIndex() {
  ...
  _searchIndex = [];
  window._searchIndex = _searchIndex; // expose for external access
  if (!window._MOD_FNS) {
  ...
  window._searchTypeahead = _searchTypeahead; // expose for external access
}

var _searchTimer = null;
var _searchTypeahead = null;
window._searchTypeahead = _searchTypeahead;
...
(function(){
  ...
  function onEnd() {
    ...
    if(!moved) {
      openEmergency();
      // Context-aware: jump to relevant protocol based on active module
      var ctxIdx = (_navMod && typeof _modToEmIdx[_navMod] === 'number') ? _modToEmIdx[_navMod] : -1;
      if(ctxIdx >= 0) {
        setTimeout(function() { showEmProtocol(ctxIdx); }, 120);
      }
    }
...
var DRUG_SUPER_GROUPS = [
  {
    icon:  ICONS.cardioEndo,
    label: 'Cardiovascular & Metabolic',
    color: '#0ea5e9',
    desc:  'Antihypertensives, vasopressors, antidiabetics, insulin, cardio & endocrine drugs',
    tabs:  ['Antihypertensives','Vasopressors','Antidiabetics','Insulin Calc','Cardio & Endocrine'],
  },
  {
    icon:  ICONS.emergencyMeds,
    label: 'Emergency & Critical Care',
    color: '#ef4444',
    desc:  'Emergency meds, IV fluids, analgesics, electrolyte replacement',
    tabs:  ['Emergency Meds','IV Fluids','Analgesics','Electrolytes'],
  },
  {
    icon:  ICONS.antibiotics,
    label: 'Infection & Tropical Medicine',
    color: '#f97316',
    desc:  'Antibiotics, TB & HIV, antimalarials',
    tabs:  ['Antibiotics','TB & HIV','Antimalarials'],
  },
  {
    icon:  ICONS.treatment,
    label: 'Specialty Drugs',
    color: '#8b5cf6',
    desc:  'Resp & GI, neuro & MSK, obs & gynae, haematology, dermatology, ophthalmology, ENT, oncology, palliative',
    tabs:  ['Resp & GI','Neuro & MSK','Obs & Gynae','Haematology','Dermatology','Ophthalmology','ENT','Oncology Support','Palliative Care'],
  },
  {
    icon:  ICONS.evaluation,
    label: 'Reference',
    color: '#d97706',
    desc:  'Drug interactions, safety in pregnancy, psychiatric drug profiles',
...
function renderDrugNav(cont) {
  // Sync active group to match the currently active tab
  ...
  if (!activeGroup) { cont.appendChild(nav); return; }

  // ── Group label row (tappable — goes back to group landing) ──
  var l1 = document.createElement('div');
  l1.style.cssText = 'display:flex;align-items:center;gap:8px;padding:7px 0 5px;';
  var backBtn = document.createElement('button');
  backBtn.style.cssText = 'display:flex;align-items:center;gap:6px;background:' + activeGroup.color + '18;' +
    'color:' + activeGroup.color + ';border:none;border-radius:20px;padding:4px 12px 4px 8px;' +
    'font-size:11.5px;font-weight:700;cursor:pointer;white-space:nowrap;';
  backBtn.innerHTML = '← ' + activeGroup.icon + ' ' + activeGroup.label;
  backBtn.onclick = function() { navToDrugGroups(); };
  l1.appendChild(backBtn);
  nav.appendChild(l1);
...
function injectDrugSearch(cont, tabBody) {
  ...
  // Calculate sticky top: below content-back-bar only (drug-nav no longer exists)
  var cbb = document.getElementById('content-back-bar');
  var cbbH = cbb ? cbb.offsetHeight : 48;
  var stickyTop = cbbH + 8;
  ...
  input.addEventListener('input', function() {
    ...
    // Always query live from DOM — avoids stale closure on re-render
    var liveBody = document.getElementById('drug-tab-body');
    var liveNoRes = document.getElementById('drug-search-noresult');
    if (!liveBody) return;

    var cards = liveBody.querySelectorAll('.card');
    var visible = 0;
    ...
    // Also show/hide secTitle and notebox elements
    liveBody.querySelectorAll('.sec-title').forEach(function(st) { st.style.display = q ? 'none' : ''; });
    liveBody.querySelectorAll('.notebox, .note-box, [class*="notebox"]').forEach(function(nb) { nb.style.display = q ? 'none' : ''; });

    if (liveNoRes) liveNoRes.style.display = (q && visible === 0) ? 'block' : 'none';
  });
}
